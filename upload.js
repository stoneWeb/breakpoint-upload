		...
		Upload.prototype = {
			...
			_getFiles: function(files){
				...
				for(;f = files[i]; i++){
					...
					if(parseInt(format(size,true)) > this.fileMax){
						msg += name+' 超出大小限制！\n';
						continue;
					}
				...
				if(html !== ''){
					if(this.listParent && this.listParent.nodeType === 1){
								...
								if(this.status == 200){
									...
									if(json.success && json.results){
										for(var key in json.results){
											var id = key.split('.')[0], o = _this._elementState, f = _this._fileObject[id];
											if(!json.results[key]){//没有上传过
												o.state(id, '等待上传').size(id, 0);
											}else{
												var n = parseInt(json.results[key] / f.file.size * 100);
												o.progress(id, n);
												if(n == 100){//已传完
													o.state(id, '<a target="_blank" href="uploads/'+ key +'">已上传过</a>').size(id, f.file.size);
													_this._fileObject[id] = null;
													delete _this._fileObject[id];
												}else{//未传完
													o.state(id, '等待续传').size(id, json.results[key]);
													f.start = json.results[key];
												}
						...
						this.error('请确认listParent是否为element！');
					}
				}
			},
			...
			_init: function(opt){
				...
				!this.listParent && this.error('请确认listParent为DOMElement！');
			...
			_addEvent: function(){
				...
				//拖放选择文件
				if(_this.dragElement && _this.dragElement.nodeType === 1){
				...
				//文件选择控件
				if(_this.selectBtn && _this.selectBtn.nodeType === 1){
